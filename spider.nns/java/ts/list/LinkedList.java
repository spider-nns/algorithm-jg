...
public class LinkedList<E> {
     ...
     * @param data
     * @param index
     * @throws Exception 向链表中指定位置的元素(0 - size), 返回新节点
     */
    public Node<E> add(E data, int index) throws Exception {
        if (index > size) throw new Exception("超出范围...");

        Node<E> cur = head;
        for (int i = 0; i < index; i++) {
            cur = cur.next;
        }
        Node<E> node = new Node(data); // 将新元素链入链表
        cur.next = node;
        size++;
        return node;
    }
    ...
    /**
     * @return 判断当前链表与目标链表是否相交(相交与否取决于尾节点是否相同)
     */
